...
**Kirona** — a community cinema & series application with a Go microservices backend. The project uses a Go workspace (go.work) monorepo structure with 9 planned microservices, only `auth` is actively being implemented.
...
# Code generation
make mocks             # Generate mockery mocks
...
Routes are registered in `handler/routes.go` via a `Mount(router, deps)` function.
